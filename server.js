...
client.on("message", async message => {
  ...
  if (message.content.match(prefixMention)) {
    return message.reply(`Hello **Stranger**! my prefix is \`${default_prefix}\``);
  }
...
fetch(`https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(message.content)}&botname=${client.user.username}&ownername=BlazeGamer
`)
    .then(res => res.json())
    .then(data => {
...
client.on('ready', () => {
var channel = client.channels.cache.get('846610097319837719');
...
client.on
client.on("ready", () => {
    client.user.setActivity(`-help / Looking after ProKai Community and other servers /Servers ${client.guilds.cache.size}`, { type: "STREAMING"})
})
